...
//            std::cout << "Test";
    ...
    /** \brief get indices of the child containing feature vector, notice that if the feature vector is not located in
    * the subspace represented by the node, it will return  the left child TODO: any better way to fix this?*/
    virtual index_t get_child_index_by_feature (const std::vector<num_t> &feature_vector) const
    {
        index_t child_idx = falls_into_child(feature_vector);
        return get_child_index(child_idx);
    }
    ...
    const std::vector<index_t>& get_data_indices() const
    {
        if (is_a_leaf())
            return data_indices;
        else
            std::runtime_error("acquiring data indices is only allowed in leaf nodes!");
    }
...
class k_ary_node_full: public k_ary_node_minimal<k, split_type, num_t, response_t, index_t, rng_t>{
   ...
   */
    ...
    /** \brief get indices of the child containing feature vector, notice that if the feature vector is not located in
    * the subspace represented by the node, it will return  the left child TODO: any better way to fix this?*/
    index_t get_child_index_by_feature (const std::vector<num_t> &feature_vector) const
    {
        return super::get_child_index_by_feature(feature_vector);
    }
