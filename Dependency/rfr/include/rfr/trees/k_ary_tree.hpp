...
class k_ary_random_tree : public rfr::trees::tree_base<num_t, response_t, index_t, rng_type> {
  ...
  public:
	...
	/* \breif Function to get the subspace split by the node given the index of the node
	 *  bounds: stores the intervals for all continuous variables, stores the number of categories for categoricals
	 * */
	virtual std::vector< std::vector<num_t> > get_subspace_by_node ( const std::vector<std::pair<num_t, num_t> > bounds, index_t node_idx) const
    {
        if (node_idx >= the_nodes.size())
        {
            throw std::runtime_error("node_idx is greater than the number of nodes!");
        }
	    index_t feature_size = bounds.size();
        std::vector< std::vector<num_t> > bounds_subspace;
        bounds_subspace.reserve(feature_size);
        for (auto it:bounds)
        {
            std::vector<num_t> bound_i;
            bound_i.reserve(2);
            bound_i.push_back(it.first);
            bound_i.push_back(it.second);
            bounds_subspace.push_back(bound_i);
        }
        if(node_idx==0)
        {
            for (index_t i=0u; i<bounds.size(); ++i)
            {
                if (bounds[i].first > 0 && std::isnan(bounds[i].second))
                {
                    num_t num_cats = bounds[i].first;
                    std::vector<num_t> bound_cat(num_cats);
                    std::iota(bound_cat.begin(), bound_cat.end(), 0);
                    bounds_subspace[i] = bound_cat;
                }
            }
            return bounds_subspace;
        }
        // here simply repeat the process in data_container/default_data_container, not sure if any better methods
        std::vector<index_t> vFeature_types(bounds.size(), 0);
        for (index_t i = 0u; i < bounds.size(); ++i)
        {
            std::pair<num_t, num_t> bound = bounds[i];
            if (bound.first > 0 && std::isnan(bound.second))
                vFeature_types[i] = bound.first;
        }
        std::vector<index_t> vAncestors;
        const node_type node = get_node(node_idx);
        index_t node_depth = node.get_depth();
        index_t parent_idx = node.parent();

        vAncestors.reserve(node_depth); // at most the number of ancestors will not be larger than the depth of the tree
        vAncestors.push_back(node_idx);
        vAncestors.push_back(parent_idx);
        while (parent_idx != 0)
        {
            parent_idx = get_node(parent_idx).parent();
            vAncestors.push_back(parent_idx);
        }
        parent_idx = vAncestors.back();
        vAncestors.pop_back();
        while (!vAncestors.empty())
        {
            index_t child_idx = vAncestors.back();
            vAncestors.pop_back();
            const node_type& parent = get_node(parent_idx);
            index_t feature_idx = parent.get_split().get_feature_index();
            std::vector<num_t>& current_bound = bounds_subspace[feature_idx];
            if (vFeature_types[feature_idx] == 0)  // num features
            {
                for (index_t i =0u; i< k; ++i)
                {
                    if (child_idx == parent.get_child_index(i))
                    {
                        if (i==0)
                        {
                            current_bound[1] = parent.get_num_split_value();
                            break;
                        }
                        else if (i==1)
                        {
                            current_bound[0] = parent.get_num_split_value();
                            break;
                        }
                    }
                }
            ...
            {
                std::vector<num_t> cat_split = parent.get_cat_split();
                for (index_t i =0u; i<k; ++i)
                {
                    if (child_idx == parent.get_child_index(i))
                    {
                        //only one element in bounds_subspace, this should not happen though...
                        if(current_bound.size()<2)
                        {
                            break;
                        }
                        if (i==0)
                        {
                            if (std::isnan(current_bound[1]))
                            {
                                // for the left child, we simply load the value into bounds when bound is
                                current_bound = cat_split;
                            }
                            else
                            {
                                // unoberserved value are assigned randomly to cat_split, to avoid that, we need to take the intersection of two values
                                std::sort(cat_split.begin(), cat_split.end());
                                std::sort(current_bound.begin(), current_bound.end());
                                std::vector<num_t> vIntersection;
                                std::set_intersection(current_bound.begin(), current_bound.end(),
                                                      cat_split.begin(), cat_split.end(),
                                                      std::back_inserter(vIntersection));
                                current_bound = vIntersection;
                            }
                            break;
                        }
                        else if (i==1)
                        {
                            ...
                            if (std::isnan(current_bound[1]))
                            {
                                std::vector<num_t> cat_right;
                                cat_right.reserve(current_bound[0]);
                                // if bounds_subspace is still in the form of (num_cat, nan)
                                auto cat_iter = cat_split.begin();
                                for (index_t j =0; j< current_bound[0]; ++j)
                                {
                                    // j in cat_split
                                    if(j==*cat_iter)
                                        ++cat_iter;
                                    else
                                        cat_right.push_back(j);
                                }
                                cat_right.shrink_to_fit();
                                current_bound = cat_right;
                            }
                            else
                            {
                                std::sort(current_bound.begin(), current_bound.end());
                                std::vector<num_t> vDiff;
                                //find the value that exists in bounds_subspace[feature_idx] but not in cat_split
                                std::set_difference(current_bound.begin(), current_bound.end(),
                                                    cat_split.begin(), cat_split.end(),
        ...
        for (index_t i=0u; i<bounds.size(); ++i)
        {
            if (bounds_subspace[i][0] > 0 && std::isnan(bounds_subspace[i][1]))
            {
                num_t num_cats = bounds_subspace[i][0];
                std::vector<num_t> bound_cat(num_cats);
                std::iota(bound_cat.begin(), bound_cat.end(), 0);
                bounds_subspace[i] = bound_cat;
            }
    ...
    /* \breif Function to get the data indices stored in the noed
 *  bounds: stores the intervals for all continuous variables, stores the number of categories for categoricals
 * */
    virtual std::vector<index_t> get_data_indices_by_node(index_t node_idx) const
    {
        std::vector<index_t> data_indices;
        data_indices.reserve(get_node(node_idx).get_num_data());
        std::vector<index_t> vLeaves;
        vLeaves.reserve(num_leafs);
        get_leaves_by_node(node_idx, vLeaves);
        for(auto leaf:vLeaves)
        {
            const std::vector<index_t> data_indices_leaf = get_node(leaf).get_data_indices();
            data_indices.insert(data_indices.end(), data_indices_leaf.begin(), data_indices_leaf.end());
        }
        return data_indices;
    }

    void get_leaves_by_node (index_t node_idx, std::vector<index_t>& vLeaves) const
    {
        const node_type& node = get_node(node_idx);
        if (node.is_a_leaf())
            vLeaves.push_back(node_idx);
        else
        {
            std::array<index_t, k> children = node.get_children();
            for (auto child=children.begin(); child!=children.end(); ++ child)
            {
                get_leaves_by_node(*child, vLeaves);
            }
        }
    }
